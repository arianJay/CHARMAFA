      ...
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <form @submit.prevent="saveInfo" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">            <div>
            ...
            <h3 class="text-lg font-semibold text-gray-800 mb-4">OR (Official Receipt) Number Management</h3>
            ...
            <div v-if="isEditing" class="mb-6">
              ...
              >
                {{ creatingBooklet ? 'Creating...' : 'Generate OR Numbers' }}
              </button>
            </div>

            <div v-if="orBooklets.length > 0" class="mb-6">
              <h4 class="text-md font-semibold text-gray-700 mb-3">OR Booklets History</h4>
                ...
                >
                  <div>
                    <p class="font-medium text-gray-800">
                      OR #{{ booklet.start_number }} - #{{ booklet.end_number }}
                    </p>
...
const loadOrBooklets = async () => {
  try {
    ...
    console.error('Failed to load OR booklets:', err)
  }
}

const createOrBooklet = async () => {
  if (!orForm.value.start_number || !orForm.value.end_number) {
    error.value = 'Please enter both start and end OR numbers'
    return
  }
  ...
  try {
    ...
    success.value = `Successfully created ${result.total_numbers} OR numbers (${orForm.value.start_number} - ${orForm.value.end_number})`
    ...
    console.error('Create OR booklet error:', err)
    error.value = err.message || 'Failed to create OR booklet. Please try again.'
  } finally {
